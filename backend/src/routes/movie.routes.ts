...
import { body, param, query, validationResult } from "express-validator";
import authenticate from "../middleware/auth";
import * as ctrl from "../controllers/movie.controller";

const router = express.Router();

// helper to wrap express-validator errors
...
/** List all */
router.get("/", authenticate, ctrl.getMovies);
...
router.delete(
  "/:id",
  authenticate,
  [param("id").isMongoId().withMessage("Invalid movie ID"), handleValidation],
  ctrl.deleteMovie
);
